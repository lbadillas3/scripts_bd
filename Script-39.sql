select 
etiquetas, count(*) from (select distinct on (dv.mdata::json->>'id_gestion')
...
when cal.mdata::json->>'cal_eval_typing' is null then 'Venta No Auditada'
when cal.mdata::json->>'cal_eval_typing' = '' then 'Venta No Auditada'
end as etiquetas
FROM (
	select  *
