SELECT  
	    ...
	    '0' as "OPERID",
	    to_char(g.ges_his_date_form , 'dd/mm/yyyy hh24:mm:ss') as "GES_FECHA",
	    ...
	    case when hfh.for_his_metadata->>'pro_value_plan' is null then '0' else hfh.for_his_metadata->>'pro_value_plan' end as "UF_POLIZAS",
	    'H' as "TIPO_GESTION"
		---------------------------------------------------------------------------------------------------------------
                    ...
                    WHERE
					to_char(g.ges_his_date_form,'yyyymmdd')::integer = to_char(now(),'yyyymmdd')::int
