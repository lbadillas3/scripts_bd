...
nomprod, replace(concat(nombre_casi_final ,uniqueid,'.mp3'),'|','') as nombre_final,
replace(concat(ruta_casi_final ,'/',uniqueid,'.mp3'),'|','')  as ruta_final
from (
...
from 
	...
	where fecha like '202110%%' and estado_venta in ('OK','RECUPERADO') and v.idunico = c.ws_idunico ) qv
left join (
