...
concat('/COPEUCH/',to_char(fecha::date,'YYYYMMDD')) as ruta_casi_final,
uniqueid, duration
from 
	(select rank() over (partition by idunico order by fecha,hora,ramo desc) as rango_ventas,idunico as idunico2 ,* 
	from cardif_coopeuch.ventas v, cardif_coopeuch.clientes c 
	where fecha like '202112%%' and estado_venta in ('OK','RECUPERADO') and v.idunico = c.ws_idunico ) qv
left join (
	...
	where fecha like concat('%%%122021','-%%:%%:%%') and idaplicacion = 2342 and disposition = 'ANSWERED')q2	 
) qcdr
