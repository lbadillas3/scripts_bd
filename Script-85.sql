select distinct on (hfh.for_his_genesys_id)
cc.con_id,
...
hfh.for_his_con_id as "INDICE TMK",--(id_grabacion o id_llamada, reemplazar por todo)
...
concat(UPPER(hfh.for_his_metadata::json->>'Nombre'),' ',replace(UPPER(hfh.for_his_metadata::json->>'SegundoNombre'), '.', ''))  as "NOMBRES",
UPPER(hfh.for_his_metadata::json->>'ApPaterno') as "APELLIDO PATERNO TIT",
replace(UPPER(hfh.for_his_metadata::json->>'ApMaterno'), '.', '') as "APELLIDO MATERNO TIT",--si es vacion o punto no enviar nada
...
(case 
when (upper(cc.con_metadata::json->>'sexo') = 'MASCULINO') then '01'
when (upper(cc.con_metadata::json->>'sexo') = 'FEMENINO') then '02'
else ''
end) as "GENERO TIT", --poner los 0
...
(case when(length(r.comuna_ccla) <3 ) then concat('0',r.comuna_ccla)
when (r.comuna_ccla is null) then hfh.for_his_metadata::json->>'ComunaId'
else r.comuna_ccla
end) as "COMUNA PARTICULAR TIT",
(case when(length(r2.ciudad_ccla) <3 ) then concat('0',r2.ciudad_ccla)
when (r2.ciudad_ccla is null) then hfh.for_his_metadata::json->>'CiudadId'
else r2.ciudad_ccla
end) as "CIUDAD PARTICULAR TIT",
'152' as "PAIS RESIDENCIA",
'152' as "NACIONALIDAD",
hfh.for_his_metadata::json->>'pro_valu_plan' as "PRIMA TIT",
'00000' as "PROFESION TIT",--codigo con 0 antepuestos no nombre
(case 
 when replace(hfh.for_his_metadata::json->>'pro_cod_plan_sponsor','0','')='31' then '03'
 when replace(hfh.for_his_metadata::json->>'pro_cod_plan_sponsor','0','')='32' then '03'
 when replace(hfh.for_his_metadata::json->>'pro_cod_plan_sponsor','0','')='41' then '04'
 ...
 concat(UPPER(hfh.for_his_metadata::json->>'Nombre'),' ',replace(UPPER(hfh.for_his_metadata::json->>'SegundoNombre'), '.', ''))  as "NOMBRES CONT",
UPPER(hfh.for_his_metadata::json->>'ApPaterno') as "APELLIDO PATERNO CONT",
replace(UPPER(hfh.for_his_metadata::json->>'ApMaterno'), '.', '')  as "APELLIDO MATERNO CONT",
...
(case when(length(r.comuna_ccla) <3 ) then concat('0',r.comuna_ccla)
when (r2.ciudad_ccla is null) then hfh.for_his_metadata::json->>'ComunaId'
else r.comuna_ccla
end) as "COMUNA PARTICULAR CONT",
(case when(length(r2.ciudad_ccla) <3 ) then concat('0',r2.ciudad_ccla)
when (r2.ciudad_ccla is null) then hfh.for_his_metadata::json->>'CiudadId'
else r2.ciudad_ccla
end) as "CIUDAD PARTICULAR CONT",
'152' as "PAIS RESIDENCIA CONT",
'152' as "NACIONALIDAD CONT",
hfh.for_his_metadata::json->>'pro_valu_plan' as "PRIMA CONT",
'00000' as "PROFESION CONT",
(case 
 when replace(hfh.for_his_metadata::json->>'pro_cod_plan_sponsor','0','')='31' then '03'
 when replace(hfh.for_his_metadata::json->>'pro_cod_plan_sponsor','0','')='32' then '03'
 when replace(hfh.for_his_metadata::json->>'pro_cod_plan_sponsor','0','')='41' then '04'
 ...
 concat(UPPER(hfh.for_his_metadata::json->>'Nombre'),' ',replace(UPPER(hfh.for_his_metadata::json->>'SegundoNombre'), '.', ''))  as "NOMBRES PAG",
UPPER(hfh.for_his_metadata::json->>'ApPaterno') as "APELLIDO PATERNO PAG",
replace(UPPER(hfh.for_his_metadata::json->>'ApMaterno'), '.', '')  as "APELLIDO MATERNO PAG",
...
end) as "COMUNA PARTICULAR PAG",
(case when(length(r2.ciudad_ccla) <3 ) then concat('0',r2.ciudad_ccla)
when (r2.ciudad_ccla is null) then hfh.for_his_metadata::json->>'CiudadId'
else r2.ciudad_ccla
end) as "CIUDAD PARTICULAR PAG",
'152' as "PAIS RESIDENCIA PAG",
'152' as "NACIONALIDAD PAG",
hfh.for_his_metadata::json->>'pro_valu_plan' as "PRIMA PAG",
'00000' as "PROFESION PAG",
------Adicional
...
concat(hah.nombre_adi1,' ',replace(segundo_nombre_adi1,'.','')) as "NOMBRES AD1",
...
concat(hah.nombre_adi2,' ',replace(segundo_nombre_adi2,'.','')) as "NOMBRES AD2",
...
concat(hah.nombre_adi3,' ',replace(segundo_nombre_adi3,'.','')) as "NOMBRES AD3",
...
concat(hah.nombre_adi4,' ',replace(segundo_nombre_adi4,'.','')) as "NOMBRES AD4",
...
then (case 
 ...
 concat(hah.nombre_adi5,' ',replace(segundo_nombre_adi5,'.','')) as "NOMBRES AD5",
...
------beneficiario
concat(hbh.nombre_benef1,' ',replace(segundo_nombre_benef1,'.',''))as "NOMBRES B1",
...
else hfh.for_his_metadata::json->>'Telefono' end) else '' end) as "MOVIL PARTICULAR B1",
...
else hfh.for_his_metadata::json->>'Telefono' end) else '' end) as "MOVIL PARTICULAR B2",
...
else hfh.for_his_metadata::json->>'Telefono' end) else '' end) as "MOVIL PARTICULAR B3",
...
else hfh.for_his_metadata::json->>'Telefono' end) else '' end) as "MOVIL PARTICULAR B4",
...
else hfh.for_his_metadata::json->>'Telefono' end) else '' end) as "MOVIL PARTICULAR B5",
...
else hfh.for_his_metadata::json->>'Telefono' end) else '' end) as "MOVIL PARTICULAR B6",
...
else hfh.for_his_metadata::json->>'Telefono' end) else '' end) as "MOVIL PARTICULAR B7",
...
where hbh.ben_his_con_id <> ''{{dialer.con_id}}''
group by hbh.ben_his_con_id) as subquery on hbh.ben_his_con_id = subquery.ben_his_con_id
left join cardif_hospitalario.regcomciu ciu on ciu.nciudad = hbh.ben_his_metadata::json->>''ComunaId''
where hbh.ben_his_con_id <> ''{{dialer.con_id}}''
union
...
left join cardif_hospitalario.tabla_parentesco tp on tp.descripcion = upper(hah.adi_his_metadata::json->>''parentesco'')
where hah.adi_his_con_id <> ''{{dialer.con_id}}''
union 
...
when hah.adi_his_metadata::json->>''Rut'' = ''11111111-1'' then '' ''
...
when split_part(hah.adi_his_metadata::json->>''Comuna'', ''-'',  1) is null then ''''
	 else split_part(hah.adi_his_metadata::json->>''Comuna'', ''-'',  1)
end
FROM "Genesys".his_aditional_history hah inner join (SELECT min(hah.adi_his_id) as mini,hah.adi_his_con_id
from "Genesys".his_aditional_history hah
where hah.adi_his_con_id <> ''{{dialer.con_id}}''
...
when split_part(hah.adi_his_metadata::json->>''Ciudad'', ''-'',  1) is null then ''''
	 else split_part(hah.adi_his_metadata::json->>''Ciudad'', ''-'',  1)
end
FROM "Genesys".his_aditional_history hah inner join (SELECT min(hah.adi_his_id) as mini,hah.adi_his_con_id
from "Genesys".his_aditional_history hah
where hah.adi_his_con_id <> ''{{dialer.con_id}}''
...
when split_part(hah.adi_his_metadata::json->>''Nacionalidad'', ''-'',  2) is null then ''''
	 else split_part(hah.adi_his_metadata::json->>''Nacionalidad'', ''-'',  2)
end
FROM "Genesys".his_aditional_history hah inner join (SELECT min(hah.adi_his_id) as mini,hah.adi_his_con_id
from "Genesys".his_aditional_history hah
where hah.adi_his_con_id <> ''{{dialer.con_id}}''
...
when split_part(hah.adi_his_metadata::json->>''Pais'', ''-'',  2) is null then ''''
	 else split_part(hah.adi_his_metadata::json->>''Pais'', ''-'',  2)
end
FROM "Genesys".his_aditional_history hah inner join (SELECT min(hah.adi_his_id) as mini,hah.adi_his_con_id
from "Genesys".his_aditional_history hah
where hah.adi_his_con_id <> ''{{dialer.con_id}}''
...
left join cardif_hospitalario.regcomciu r on r.ncomuna =  hfh.for_his_metadata::json->>'ComunaId'
left join cardif_hospitalario.regcomciu r2 on r2.nciudad =  hfh.for_his_metadata::json->>'CiudadId'
where hgh.ges_his_tipificacion_4 in ('ARGUMENTADO SI','VENTA RECUPERADA')
and hfh.for_his_metadata::json->>'Rut' in ('11506083-K','26570019-5','17960924-K','15740278-1','10571158-1','18649949-2','9240476-5','18036007-7','13125081-9','10061209-7','9072397-9','9508286-6','10177238-1','17856737-3','13706777-3','17765005-6')
...
--and to_char(hfh.for_his_date_form::date,'yyyymmdd') between  '20210901'  and '20211231'
